...
function useBankCardValidator() {
  const formatBankCard = react.useCallback((card) => {
    const digits = normalizeDigits(card).slice(0, 16);
    return digits.replace(/(.{4})/g, "$1 ").trim();
  }, []);
  const isValidBankCard = react.useCallback((card) => {
    const digits = normalizeDigits(card);
    if (digits.length !== 16) return false;
    if (/^(\d)\1{15}$/.test(digits)) return false;
    return luhnCheck(digits);
  }, []);
  const getBankInfo = react.useCallback((card) => {
    const digits = normalizeDigits(card);
...
function useNumberToWordsFa() {
  const numberToWordsFa = react.useCallback((num) => {
...
function usePersianDate() {
  const toDateFa = react.useCallback(
    (date, opts) => {
...
function useRelativeTimeFa(autoUpdate = false) {
  ...
  const toRelativeTime = react.useCallback(
    (input) => {
...
function useTypingLanguageFa() {
  const [isPersianText, setIsPersianText] = react.useState(false);
